...
uv run next-signal <cmd>        # run the CLI
...
3. Reference models by profile name (`configs/models.yaml`) and tools by
   registered name.
4. Restart `next-signal serve`, or smoke-test with `next-signal run-agent <name> "..."`.
...
- **Cross-cutting tool** (useful in any domain, e.g. GBrain search) →
  `src/next_signal/tools/`, plus one line in `next_signal/registry.py::_IN_TREE_TOOLS`.
- **Domain tool** → `src/next_signal/tools/<domain>/`, registered via
...
- **Generic provider** (any domain might use it) → `src/next_signal/integrations/`,
  plus one line in `_MODULES` in `integrations/__init__.py`.
- **Domain provider** → `src/next_signal/integrations/<domain>/`, called by that
...
a workflow. Workflows are centralized in `src/next_signal/workflows/` and declared by
`configs/workflows/<name>.yaml`.
...
`src/next_signal/workflows/stages/<workflow>/`. An action reusable by several workflows
...
- `expose.agent_os: true` → `next_signal.os_app` registers it with AgentOS via the
  centralized loader.
- `expose.tool.enabled: true` → `next_signal.orchestrator.workflow_tools` registers a
  `WorkflowTools` toolkit; agent YAML references that tool name.
- `extra.run_now` → the function `next-signal run-workflow <name>` calls for a manual
...
complex routing justifies a `src/next_signal/teams/<name>.py` factory. No team ships
...
`modules/` layer: `src/next_signal/tools/<domain>/`,
`src/next_signal/integrations/<domain>/`, and where needed
`src/next_signal/workflows/<name>.py` plus `configs/{agents,workflows,teams}/`.
