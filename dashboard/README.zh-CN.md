...
`engine.json` 里没设过的字段会回落到 `configs/models.yaml` 的值，所以全新安装看到的是
真实的模型，而不是 dashboard 编出来的默认值。本地端点是例外：它没有任何 baseline，
初始就是**空的**——仓库拿不出一个正确的值，而从本容器的环境里读一个，等于把 dashboard
自己的答案显示成 scheduler 的答案。端点留空是可以保存的——这就是「我没有本地服务」的
表达方式，OMLX profile 会因此走云端回落。改动对定时任务和新命令立即生效；AgentOS 里
...
**全新安装什么都不选中**，这一节会在卡片上方写明：在你选一个 embedder 之前去重是关着的，
雷达其余部分照常运行。仓库没有一个能替你老实挑的 provider——本地那个要一个只有你知道的
地址，云端那两个要密钥、要花钱。

三个平级选项——**本地模型**、**OpenAI**、**自定义端点**——行为完全一致：
...
保存下来的东西才会跑。**保存**会存下该 provider 的字段，并在同一次写入里选中它。
换 provider 一定会先问，因为这会把当前向量空间下记住的所有主题搁置起来。
...
没有任何凭据经过这个页面，任何一节甚至都不提凭据的名字：每个 provider 的密钥在凭据库里
都有固定名字（`OPENAI_API_KEY`、`EMBEDDING_API_KEY`）。URL 被限制为纯 API 根地址，
所以密钥没法藏在 userinfo、query 或 fragment 里被存下来。密钥在不在是服务端算好的，
传到浏览器的只是一个布尔值。在**设置 → 凭据**里存的密钥下一条就生效——不用重启，也不用
重建 Compose 服务——因为凭据库是在 item 解析 embedder 时才读的。云端 provider 还会
