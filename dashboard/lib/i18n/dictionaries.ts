...
export const dictionaries = {
  en: {
    ...
    goals: {
      title: "Goals",
      tabs: { radar: "Info Radar", content: "Content" },
      subtitle: (count: number) =>
        `Steers Tier-1 filtering & scoring · info_radar/goals.yaml · ${count} goals`,
      subtitleContent: (count: number) =>
        `Steers topic discovery · content/goals.yaml · ${count} goals`,
    ...
    smartMoney: {
      title: "Smart Money",
      tagline: "Research leads, not trade signals — all disclosures are lagged",
      pull: "Pull",
      analyze: "Analyze",
      pulling: "Pulling",
      analyzing: "Analyzing",
      sumLastPull: "Last pull",
      sumSources: "Events by source",
      sumSignals: "Signals",
      sumUnmapped: "Unmapped",
      sumTrend: "Pulled · last 30d",
      neverPulled: "never",
      scoreRaw: "raw score",
      unmappedTitle:
        "ticker is NULL (CUSIP mapping failed) — shown by issuer_name, excluded from resonance",
      tabSignals: "Signals",
      tabEvents: "Events",
      filters: "Filters",
      clearAll: "Clear",
      ...
      fMore: "More",
      fCreated: "Created",
      tickerPh: "ticker prefix…",
      actorPh: "Search actor name…",
      fRule: "Rule",
      fConf: "Confidence",
      fSource: "Source",
      fType: "Type",
      fAction: "Action",
      fFresh: "Freshness",
      fOnlyTicker: "Ticker only",
      fVmin: "Min value",
      confHigh: "high",
      confMedium: "medium",
      confLow: "low",
      confMissing: "no analysis",
      hypothesis: "Hypothesis",
      relevance: "Relevance to goals",
      nextSteps: "Research next steps",
      evidence: "Underlying events",
      degradedTitle: "LLM analysis failed — rule score only",
      degradedBody:
        "tier2 interpretation degraded; the signal itself is still valid. The evidence events that triggered it are shown below.",
      colActor: "actor",
      colAction: "action",
      colAmount: "value",
      colTrade: "trade date",
      colDisclosed: "disclosed",
      colLag: "lag",
      colSource: "source",
      colType: "type",
      colTicker: "ticker",
      ...
      unmapped: "unmapped",
      emptySignals:
        "No signals under the current filter. Resonance needs a cross-source or multi-insider same-direction evidence set.",
      emptyEvents:
        "No events under the current filter. The ptr (congressional) source is disabled — expect 0 rows.",
      ptrTitle:
        "ptr (congressional PTR): all free sources went dead in 2026-07. Kept but disabled by default; expect 0 rows.",
      shownOf: (shown: number, total: number) => `${shown} / ${total} shown`,
      actions: {
        pullStarted: "Started: pull (smart-money pull)",
        analyzeStarted: "Started: analyze (smart-money analyze)",
        pullAlreadyRunning: "Pull already running",
        analyzeAlreadyRunning: "Analyze already running",
      },
    },
  },
  zh: {
    ...
    goals: {
      title: "目标",
      tabs: { radar: "信息雷达", content: "内容创作" },
      subtitle: (count: number) =>
        `控制 Tier-1 过滤与评分 · info_radar/goals.yaml · ${count} 个目标`,
      subtitleContent: (count: number) =>
        `驱动选题发现 · content/goals.yaml · ${count} 个目标`,
    ...
    smartMoney: {
      ...
      scoreRaw: "原始分",
      unmappedTitle:
        "ticker 为空（CUSIP 映射失败），以 issuer_name 展示，不参与共振",
      tabSignals: "信号",
      tabEvents: "事件",
      filters: "筛选器",
      clearAll: "清除",
      ...
      nextSteps: "研究下一步",
      evidence: "事件明细",
      degradedTitle: "LLM 解读失败，仅规则打分",
      degradedBody:
        "tier2 解读降级，信号本身仍有效；下方为触发该信号的事件证据。",
      colActor: "actor",
      colAction: "动作",
      colAmount: "金额",
      colTrade: "交易日",
      ...
      fresh3: ">45 天",
      unmapped: "unmapped",
      emptySignals: "当前筛选下没有信号。共振需要跨源或多内部人同向证据集合。",
      emptyEvents: "当前筛选下没有事件。ptr（国会披露）源已禁用，预期 0 行。",
      ptrTitle:
        "ptr（国会披露）：所有免费源已于 2026-07 失效，源保留但默认禁用，预期 0 行。",
      shownOf: (shown: number, total: number) => `显示 ${shown} / ${total}`,
      actions: {
        pullStarted: "已启动：拉取（smart-money pull）",
        analyzeStarted: "已启动：解读（smart-money analyze）",
        pullAlreadyRunning: "拉取正在进行中",
        analyzeAlreadyRunning: "解读正在进行中",
      },
    },
  },
} as const;
