...
def _register_all() -> None:
    ...
    for mod_name, names in _IN_TREE_TOOLS.items():
        try:
            mod = __import__(f"next_signal.tools.{mod_name}", fromlist=names)
    ...
    for package in _TOOL_PACKAGES:
        try:
            mod = __import__(f"next_signal.tools.{package}", fromlist=["register"])
            mod.register(_RegistryFacade())
        except Exception as e:  # noqa: BLE001
            log.warning("tool_package_register_failed", package=package, error=str(e))

    from next_signal.orchestrator.workflow_tools import register_workflow_tools
    register_workflow_tools(_RegistryFacade())

    from next_signal.integrations import register_all
