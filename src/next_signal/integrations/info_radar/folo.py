...
``folocli`` is invoked via subprocess from ``next_signal.collectors.info_radar.runner``;
...
- ``whoami()`` — used by ``next-signal doctor`` to verify auth.
...
def whoami(timeout: float = 60.0) -> tuple[bool, str]:
    """Run ``folocli whoami`` and return (ok, message). Used by ``next-signal doctor``.
