...
invoked by ``next-signal knowledge review``.
"""
...
from next_signal.workflows.knowledge_review import store
...
from next_signal.workflows.knowledge_ingest import _markdown_files
...
def run() -> dict[str, Any]:
    ...
    Returns ``{enrolled, unenrolled, due}``. Invoked by ``next-signal knowledge review``
    and ``next-signal run-workflow knowledge_review``.
    """
...
def factory():
    ...
    raise NotImplementedError(
        ...
        "extra.run_now (`next-signal knowledge review` or `next-signal run-workflow`)."
    )
