...
from next_signal.integrations._helpers import http_client
from next_signal.integrations.knowledge.bilibili import extract_bilibili
from next_signal.integrations.knowledge.github import extract_github
from next_signal.integrations.knowledge.opencli import opencli_weixin_download
from next_signal.integrations.markitdown import convert_source
from next_signal.workflows.stages.knowledge_ingest.artifact import KnowledgeArtifact, validate_category
from next_signal.workflows.stages.knowledge_ingest.classify import detect_source_type
from next_signal.workflows.stages.knowledge_ingest.raw_store import copy_raw_file, raw_dir, write_raw_text
...
# resolve via the `en` fallback at the end. Override with YOUTUBE_TRANSCRIPT_LANGS
# (comma-separated, e.g. "ja,en").
_DEFAULT_YT_LANGS = ["zh-Hans", "zh-CN", "zh", "zh-Hant", "zh-TW", "en"]


def _youtube_transcript_languages() -> list[str]:
    raw = os.environ.get("YOUTUBE_TRANSCRIPT_LANGS", "").strip()
    if not raw:
