...
- FROM: `### Requirement: `paca list` enumerates configured runnables`
- TO: `### Requirement: `next-signal list` enumerates configured runnables`
- FROM: `### Requirement: `paca serve` starts AgentOS`
- TO: `### Requirement: `next-signal serve` starts AgentOS`
- FROM: `### Requirement: `paca run-agent` executes a single prompt`
- TO: `### Requirement: `next-signal run-agent` executes a single prompt`
- FROM: `### Requirement: `paca doctor` self-checks the environment`
- TO: `### Requirement: `next-signal doctor` self-checks the environment`
- FROM: `### Requirement: `paca info-radar pull` invokes the collector`
- TO: `### Requirement: `next-signal info-radar pull` invokes the collector`
- FROM: `### Requirement: `paca info-radar sweep` deletes expired rows`
- TO: `### Requirement: `next-signal info-radar sweep` deletes expired rows`
- FROM: `### Requirement: `paca info-radar analyze` runs the analysis pipeline`
- TO: `### Requirement: `next-signal info-radar analyze` runs the analysis pipeline`
- FROM: `### Requirement: `paca info-radar subscriptions` lists Folo subscriptions`
- TO: `### Requirement: `next-signal info-radar subscriptions` lists Folo subscriptions`
- FROM: `### Requirement: `paca dashboard` wraps the Next.js dev/build/start commands`
- TO: `### Requirement: `next-signal dashboard` wraps the Next.js dev/build/start commands`
- FROM: `### Requirement: `paca run-workflow` runs a workflow's manual entrypoint`
- TO: `### Requirement: `next-signal run-workflow` runs a workflow's manual entrypoint`
- FROM: `### Requirement: `paca knowledge` subcommand group manages ingestion and GBrain`
- TO: `### Requirement: `next-signal knowledge` subcommand group manages ingestion and GBrain`
- FROM: `### Requirement: `paca info-radar recap` generates a range recap`
- TO: `### Requirement: `next-signal info-radar recap` generates a range recap`
...
- **THEN** `next-signal doctor` reports a ✗ for the folocli check and points to `folo login` or `FOLO_TOKEN`
