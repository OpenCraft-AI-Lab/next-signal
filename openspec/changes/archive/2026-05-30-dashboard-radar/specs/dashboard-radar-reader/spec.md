...
The `/radar/[id]` page AND each `/radar` item card SHALL expose an "Ingest to wiki" action that runs `uv run paca knowledge ingest <url>` for the item's `radar_items.url` via a Next.js server action. The server action SHALL re-fetch `url` from DB by `radar_items.id` (never trusting a client-passed URL), validate it via `new URL(...)`, and report outcome via `sonner` toast.

#### Scenario: ingest fires and toasts

- **WHEN** the operator clicks "Ingest to wiki" on an item with a valid `url`
- **THEN** the server action invokes `execFile("uv", ["run", "paca", "knowledge", "ingest", url])` with `cwd` at the repo root, returns immediately without awaiting stdout, and a `sonner` toast confirms the ingest started

#### Scenario: invalid URL is rejected

- **WHEN** the operator clicks "Ingest to wiki" on an item whose `url` is `NULL` or does not parse as a valid `URL`
- **THEN** the action does NOT spawn a subprocess and the toast shows an error explaining the URL is missing or malformed

### Requirement: Combined `Pull + Analyze` trigger

The dashboard top nav SHALL render a single primary `Pull + Analyze` button (mounted into foundation's `<NavTriggerSlot />`) that runs a Next.js server action which (1) synchronously awaits `uv run paca info-radar pull`, records pull start/completion timestamps and inserted-row count in `radar-state.json`, then (2) `spawn`s `uv run paca info-radar analyze` detached + unref'd and records the analyze spawn timestamp. The action SHALL return one `sonner` toast confirming pull completion and analyze kickoff.

#### Scenario: combined run kicks off both phases

- **WHEN** the operator clicks `Pull + Analyze`
- **THEN** the action awaits `paca info-radar pull` to completion, then spawns `paca info-radar analyze` detached with `stdio: "ignore"` and `.unref()`, and returns `{ ok: true, message: "Pull complete · analyze started" }`

#### Scenario: button reflects in-flight phase

- **WHEN** the action is in-flight
- **THEN** the button label reads `Pulling…` while pull is awaited; after the action returns, the label reads `Analyzing…` for ~6 seconds (client-side timer) before returning to default

#### Scenario: pull failure surfaces in the toast

- **WHEN** `paca info-radar pull` exits non-zero
- **THEN** the action returns `{ ok: false, message: <error excerpt> }`, the `Analyzing…` phase does NOT start, and the toast shows the error

#### Scenario: refresh reflects new state

- **WHEN** the operator refreshes `/radar` after a triggered run completes
- **THEN** the tracker counts and item list reflect the new rows in `radar_items` / `radar_analyses` — no extra UI is required to track the analyze phase's progress
