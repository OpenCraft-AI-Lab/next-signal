...
- **THEN** the page reads `configs/info_radar/goals.yaml` and renders one card per goal with name, description, weight, topic count, and keyword count
...
- **WHEN** the operator changes a goal description, topics, keywords, or weight and clicks Save
...
- **WHEN** the operator creates a goal with a unique kebab-case `name`, non-empty `description`, topics, keywords, and optional numeric `weight`
...
The dashboard SHALL preserve the same schema contract enforced by `paca.workflows.info_radar_analysis.goals.load_goals`: top-level `goals` list, entry fields `name`, `description`, `topics`, `keywords`, optional `weight`, unique names, and no unknown keys.
...
- **WHEN** submitted goal data includes a key outside `name`, `description`, `topics`, `keywords`, and `weight`
- **THEN** the dashboard rejects the save, does not write `goals.yaml`, and shows a validation error
