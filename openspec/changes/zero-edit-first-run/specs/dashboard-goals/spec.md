...
**Reason**: Its `empty goals list is rejected` scenario is inverted by this
change, so the requirement cannot be carried forward as a MODIFIED block without
either restating a scenario that is now false or silently dropping it. Replaced
by `Goals schema validation`, which keeps every other clause verbatim.

**Migration**: None for operators — the validation rules are unchanged except that
a zero-length `goals` list is now written instead of refused. Callers that relied
on the dashboard rejecting an empty save must rely on the loader's `RuntimeError`
instead, which is where that guarantee now lives.
